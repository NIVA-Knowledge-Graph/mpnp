...
chemistry = Chemistry(pubchem_directory='../pubchem/', load_rdf=False, endpoint='http://localhost:8890/sparql', verbose=True)
...
for f in files:
    ...
    mapping = {c:m for c,m in zip(chemicals, chemistry.convert_ids('cas','inchikey',chemicals)) if m != 'no mapping'}
