...
def create_data_structure():
    ...
    for chemical, score in zip(xtr, ytr):
        ...
        pass
        triples = [(s, p, o) for s, p, o, step in neighbors if step <= MAX_STEPS]
        steps = [step for s, p, o, step in neighbors if step <= MAX_STEPS]
        for triple, step in zip(triples, steps):
            #weight = 2**(MAX_STEPS-step)
            kg_dict[triple] = {'score': kg_dict[triple]['score'] + [score[0]],
                               'weight': kg_dict[triple]['weight'] + [2**(MAX_STEPS-step)],
                               'touched': kg_dict[triple]['touched'] + 1}

    #[sum(x) / len(x) for x in zip(*a)]
    pass # value_when_true if condition else value_when_false
    ...
    kg.to_csv('kg/sorted_kg_w_touched_descending_influence_avg.csv')
